...
var Pinia = (function (exports, compositionApi) {
  ...
  function createPinia() {
      ...
      const state = compositionApi.ref({});
      const _p = [];
  ...
  const getActivePinia = () => {
      /* istanbul ignore if */
      if (!activePinia) {
          console.warn(`[🍍]: getActivePinia was called with no active Pinia. Did you forget to install pinia and inject it?\n\n` +
  ...
  function initStore($id, buildState = () => ({}), initialState) {
      ...
      compositionApi.set(pinia.state.value, $id, initialState || buildState());
      ...
      function $subscribe(callback) {
          ...
          const stopWatcher = compositionApi.watch(() => pinia.state.value[$id], (state) => {
              if (isListening) {
                  callback({
                      storeName: $id,
                      storeId: $id,
  ...
  function buildStoreToUse(partialStore, descriptor, $id, getters = {}, actions = {}, options) {
      ...
      for (const getterName in getters) {
          ...
          computedGetters[getterName] = compositionApi.computed(() => {
              setActivePinia(pinia);
      ...
      const store = compositionApi.reactive(assign({}, partialStore, 
  ...
  function defineStore(options) {
      ...
      function useStore(pinia) {
          const hasInstance = compositionApi.getCurrentInstance();
          ...
          pinia = pinia || (hasInstance && compositionApi.inject(piniaSymbol));
          if (pinia)
              setActivePinia(pinia);
          ...
          if (!storeAndDescriptor) {
              ...
              if (shouldProvide) {
                  compositionApi.provide(storeAndDescriptor[2], store);
              }
              return store;
          }
          return ((hasInstance && compositionApi.inject(storeAndDescriptor[2], null)) ||
